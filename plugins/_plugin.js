...
Asena.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC}, (async (message, match) => {
    if (match[1] === '') return await message.sendMessage("I need a URL, like " + '.install https://gist.github.com/phaticusthiccy/4232b1c8c4734e1f06c3d991149c6fbd')
    try {
...
Asena.addCommand({pattern: 'remove(?: |$)(.*)', fromMe: true, desc: Lang.REMOVE_DESC}, (async (message, match) => {
    ...
    if (plugin.length < 1) {
        ...
        await message.sendMessage('💬 *MarvinAI is restarting automatically!*');
