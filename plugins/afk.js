...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
    if ("howdy! I'M Currently AwayFromKeyboard!" == 'default') {
            ...
            if (message.jid.includes('-') && (message.mention !== false && message.mention.length !== 0)) {
                message.mention.map(async (jid) => {
                    if (message.client.user.jid.split('@')[0] === jid.split('@')[0]) {
                        await message.client.sendMessage(message.jid,"Howdy! I'm currently AwayFromKeyboard!" + '\n' + 
                        (AFK.reason !== false ? '\n*' + "Reason" + ':* ```' + AFK.reason + '```' : '') + 
                        (AFK.lastseen !== 0 ? '\n*' + "I was last online" + ':* ```' + secondsToHms(Math.round((new Date()).getTime() / 1000) - AFK.lastseen) + ' ago```' : ''), MessageType.text, {quoted: message.data});            
                    }
                })
...
Asena.addCommand({pattern: 'afk ?(.*)', fromMe: true, deleteCommand: false, desc: Lang.AFK_DESC}, (async (message, match) => {     
    if (!AFK.isAfk) {
        ...
        await message.client.sendMessage(message.jid,"I'm now AFK!" + (AFK.reason !== false ? ('\n*' + "For that reason" +':* ```' + AFK.reason + '```') : ''),MessageType.text);
    }
}));
